...
def test_home_empty(client):
  r = client.get("/")
  assert r.status_code == 200; assert "No accounts tracked yet" in r.text
...
def _seed_digest(db_path):
  ...
  db.save_digest(c, aid, "2026-06-29T00:00:00Z", "2026-07-06T00:00:00Z", items, 0.02)
  return aid

def test_digest_page_renders(client):
  _seed_digest(client.db_path)
  c = db.connect(client.db_path)
  did = db.list_digests(c)[0]["id"]
  r = client.get(f"/digests/{did}")
  assert r.status_code == 200
  assert "hello world" in r.text; assert "$0.020" in r.text

def test_rss_feed(client):
  aid = _seed_digest(client.db_path)
...
def test_missing_pages_404(client):
  assert client.get("/digests/999").status_code == 404
