...
- **Owner actions on X:** Adding a tracked account triggers a follow from the signed-in owner account (POST follow; no pre-check of the following list). After each weekly digest is built, tweets that made it into the digest are queued and a background thread drains the queue: first like immediately, then ~1 minute ± 1–20s between each until done. OAuth tokens persist in `oauth_session` (refreshed while draining). Already-liked tweet IDs are stored in `liked_tweets`; pending likes live in `like_queue`. On app startup, a non-empty queue resumes draining.
