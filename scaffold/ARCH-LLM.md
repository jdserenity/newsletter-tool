...
- **Tweet like/dislike:** desktop — ✓ and X grouped side by side (left of meta row). Mobile (≤700px) — one row: ✓ left, meta centered, X right. Check → like on X + `liked_tweets` + mark read. X → `disliked_tweets` only. Re-click → unlike + clear + unread. Dim + sort bottom.
